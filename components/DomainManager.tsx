  ...
  const addDomain = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        addLog(`❌ 绑定失败: ${data.errors?.[0]?.message || 'API 错误'}`);
      }
    } catch (err) {
      addLog(`⚠️ 网络异常: ${err}`);
    } finally {
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium opacity-70">基于 Apple 设计规范的 Cloudflare Pages 调度系统</p>
        </div>
        <button 
        ...
        <div className="p-8 bg-white/40 dark:bg-white/5 rounded-[40px] border border-white/20 min-h-[350px] flex flex-col shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scroll">
            ...
            {domains.length === 0 && !loading && (
              <div className="flex-1 flex items-center justify-center text-slate-400 italic text-sm">暂无绑定域名</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] px-4">系统实时日志</label>
