    ...
    <div className="h-screen w-screen flex items-center justify-center bg-slate-900 overflow-hidden relative">
      ...
      <div className="relative glass w-full max-w-sm p-10 rounded-[40px] shadow-2xl text-center space-y-8 animate-fade-in border-white/10">
        <img 
          ...
          alt="Logo" 
          className="w-20 h-20 mx-auto rounded-3xl shadow-xl rotate-3 object-cover" 
        />
        
        <div className="space-y-2">
          <h2 className="text-3xl font-bold text-white tracking-tight">子怡vpn面板</h2>
          <p className="text-slate-400 text-sm">云端调度 实时控制</p>
        </div>

        <div className="space-y-4">
          <input 
            type="password"
            className={`w-full bg-black/40 border ${error ? 'border-red-500' : 'border-white/10'} rounded-2xl px-5 py-4 text-center outline-none focus:ring-2 ring-blue-500 transition-all text-white`}
            placeholder="输入访问密码"
            value={password}
          ...
          {error && <p className="text-red-500 text-xs font-semibold animate-pulse">验证失败</p>}
        </div>

        <div className="pt-4">
          <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Secure Access System v2.2</p>
        </div>
      </div>
    </div>
  );
